...
\textbf{Note for self (not reader): Want to add a sentence somewhere here that calls back to \ref{sec:algorithm} and how multi-class is like an ensemble of three classifiers, so most evaluation is per sub-predicitor}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    % \caption{Never smoker vs current smoker sub-predicitors performance (Cohort 1)}
    ...
    \end{subfigure}
    % \caption{Never smoker vs ex-smoker sub-predicitors performance (Cohort 1)}
    ...
    \end{subfigure}
    % \caption{Ex-smoker vs current smoker sub-predicitors performance (Cohort 1)}
