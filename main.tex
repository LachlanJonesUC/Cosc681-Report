...
% Both datasets required preparation before training. Raw Illumina array text files came in a format suitable for an EWAS, but not machine learning 
...
\begin{enumerate}
    \item Kruskal-Wallis: \(\sim\)480k sites \(\rightarrow\) 3122 sites
    \item Training: 3122 sites \(\rightarrow\) \(\rightarrow\) 2381 sites
    \item Performance on Cohort 1 test set
    \item Comparison to existing models
    \begin{enumerate}
...
\subsection{Cohort 2: External Validation Cohort} \label{sec:val-cohort}
